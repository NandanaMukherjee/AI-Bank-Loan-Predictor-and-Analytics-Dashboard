...
function App() {
  ...
  loans.forEach((loan) => {
    ...
    const channel =
      loan.applicationChannel || "Unknown";
  ...
  const channelData = Object.keys(channelMap).map(
    (key) => ({
      name: key,
  ...
  const purposeData = Object.keys(purposeMap).map(
    (key) => ({
      name: key,
  ...
  const defaultData = Object.keys(defaultMap).map(
    (key) => ({
      name: key,
  ...
  const branchData = Object.keys(branchMap).map(
    (key) => ({
      name: key,
      value: branchMap[key]
    })
  ...
  const employmentData = Object.keys(
    employmentMap
  ).map((key) => ({
    name: key,
  ...
  const educationData = Object.keys(
    educationMap
  ).map((key) => ({
    name: key,
  ...
  const regionData = Object.keys(regionMap).map(
    (key) => ({
      name: key,
      value: regionMap[key]
    })
  ...
  const ageData = Object.keys(ageMap).map(
    (key) => ({
      name: key,
      value: ageMap[key]
    })
  ...
  loans.forEach((loan) => {
    ...
    dependentMap[dep] =
      (dependentMap[dep] || 0) + 1;

  });

  const dependentData = Object.keys(
    dependentMap
  ).map((key) => ({
    name: key,
  ...
  const incomeLoanData = loans.map(
    (loan) => ({
      income: loan.income,
  ...
  const tenureData = loans.map((loan) => ({
    tenure: loan.loanTenureMonths,
    loanAmount: loan.loanAmount
  }));

  const fraudPatternData = loans.map(
    (loan) => ({
      applications:
        loan.multipleApplicationsCount,
      fraud:
        loan.isFraudSuspected
          ? 1
          : 0
    })
  );
...
loans.forEach((loan) => {
  ...
  const apps =
    loan.multipleApplicationsCount || 0;
...
const fraudSummaryData =
  Object.keys(fraudSummaryMap).map(
    (key) => ({
      applications: key,
